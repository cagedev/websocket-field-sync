...
class EntangledConnection {
    ...
    statusElementId;
    // statusTranslator // Array for mapping status values of element properties
    // Example stateTranslator for a form status field. Using innerHTML would be the way to go for a div status field.
    stateTranslator = EntangledConnection.colorStateTranslator;
    
    // Example stateTranslators
    static textStateTranslator = {
        0: {
            property: 'value',
            value: 'CONNECTING',
        },
    ...
    static colorStateTranslator = {
        0: {
            property: 'style',
            value: 'width: 100px; height: 100px; background-color: grey;',
        },
        1: {
            property: 'style',
            value: 'width: 100px; height: 100px; background-color: green;',
        },
        2: {
            property: 'style',
            value: 'width: 100px; height: 100px; background-color: darkblue;',
        },
        3: {
            property: 'style',
            value: 'width: 100px; height: 100px; background-color: lightgrey;',
        },
    }
    ...
    constructor(element, serverUri, token, options = {}) {
        ...
        if (options) {
            ...
            this.statusElementId = options[`${LIBRARY_PREFIX}StatusElementId`] || this.statusElementId;
        }
        ...
        this.wfsConnection.onopen = (event) => {
            this.showStatus();
        }

        this.wfsConnection.onclose = (event) => {
            this.showStatus();
        }
    ...
    close() {
        this.wfsConnection.close();
    }

    setStateTranslator(obj) {
        this.stateTranslator = obj;
    }

    showStatus() {
        if (this.statusElementId) {
            let _element = document.getElementById(this.statusElementId);
            let _state = this.wfsConnection.readyState;
            if (this.stateTranslator[_state]) {
                _element.setAttribute(
                    this.stateTranslator[_state].property,
                    this.stateTranslator[_state].value
                )
            } else {
                console.log(`[warning] stateTranslator - unknown state: ${_state}`);
            }
        } else {
            console.log(`[warning] statusElementId not defined.`);
        }
